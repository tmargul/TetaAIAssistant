...
import type { Stage2dBatchRequest, Stage2dBatchResult, Stage2dResult } from './teta-stage2d.types';
...
export function readStage2dBindings(options: {
  dllPath: string;
  match: string[];
  searchRoots?: string[];
}): Stage2dResult {
  const args = ['--stage2d', '--dll', options.dllPath];
  ...
  return JSON.parse(runReaderProcess(args)) as Stage2dResult;
}

export function readStage2dBatch(request: Stage2dBatchRequest): Stage2dBatchResult {
  const raw = runReaderProcess(['--batch-stage2d-stdin'], JSON.stringify(request));
  return JSON.parse(raw) as Stage2dBatchResult;
}
