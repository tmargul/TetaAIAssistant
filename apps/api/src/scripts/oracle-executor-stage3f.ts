...
function syncSessionContext(repoRoot: string, summary: Record<string, unknown>): void {
  ...
  const block = [
    ...
    `- connections opened/closed: ${String(summary.connectionsOpened ?? '—')} / ${String(summary.connectionsClosed ?? '—')}`,
  ...
  if (args.cmd === 'audit') {
    ...
    if (args.live) {
      syncSessionContext(repoRoot, {
        ...
        connectionsOpened: report.liveAudit.oracleConnectionsOpened,
        connectionsClosed: report.liveAudit.oracleConnectionsClosed,
      ...
      syncSessionContext(repoRoot, {
        ...
        columnCount: null,
        sqlSha256: null,
        connectionsOpened: report.offlineAudit.oracleConnectionsOpened,
        connectionsClosed: 0,
        xlsxFileName: null,
        xlsxFileSha256: null,
        parsebackOk: report.offlineAudit.fixtureXlsxParsebackOk,
      });
    }
    out(
      {
        ...
        counters: {
          ...
          connectionsClosed: report.counters.connectionsClosed,
          openOracleConnectionsAfterRun: report.counters.openOracleConnectionsAfterRun,
          resultSetsOpened: report.counters.resultSetsOpened,
          resultSetsClosed: report.counters.resultSetsClosed,
