...
import { escapeLikePrefix } from './teta-p1-vertical-pilot.types';
import { byRole } from './teta-p1-current-position-resolve';
import type { CurrentPositionResolvedBinding } from './teta-p1-current-position.types';
import {
  P1_CURRENT_POSITION_DICTIONARY,
  P1_CURRENT_POSITION_SOURCE,
  P1_SURNAME_POSITION_INTENT,
  P1_SURNAME_POSITION_MAX_ROWS,
  P1_SURNAME_POSITION_PREFIX,
  P1_SURNAME_POSITION_SCENARIO_ID,
  P1_SURNAME_POSITION_SUBJECT,
  P1_VERTICAL_OBJECT,
  P1_VERTICAL_OWNER,
  sha256SurnamePosition,
...
export function buildSurnamePositionLogicalRequest(input: {
  question: string;
  bindings: CurrentPositionResolvedBinding[];
}) {
  return {
    contractVersion: 'teta-aia-p1-surname-current-position-logical-request-v1',
    ...
    filter: {
      ...
      value: P1_SURNAME_POSITION_PREFIX,
    },
    temporalContext: {
...
export function buildSurnamePositionQueryPlan(input: {
  logicalRequest: ReturnType<typeof buildSurnamePositionLogicalRequest>;
  bindings: CurrentPositionResolvedBinding[];
}) {
  return {
    contractVersion: 'teta-aia-p1-surname-current-position-query-plan-v1',
    scenarioId: P1_SURNAME_POSITION_SCENARIO_ID,
    planStatus: 'ready_for_compilation' as const,
    intent: P1_SURNAME_POSITION_INTENT,
    subject: P1_SURNAME_POSITION_SUBJECT,
    pilotOnly: true,
    drivingSource: 'employee',
    sources: [
      {
        sourceRole: 'employee',
        status: 'resolved',
        usage: 'driving_row_source',
        accessObject: {
    ...
    whereAllowed: ['employee_last_name_starts_with'],
...
export function compileSurnamePositionSelect(input: {
  bindings: CurrentPositionResolvedBinding[];
  ...
  joinPlan: Record<string, unknown>;
} {
  ...
  const sqlText = [
    ...
    `  ${POS}.${posEmpRef.physicalColumn} AS CURRENT_POSITION_REF`,
    ...
    `WHERE UPPER(${EMP}.${last.physicalColumn}) LIKE :P001 ESCAPE '\\'`,
    `ORDER BY ${EMP}.${last.physicalColumn} ASC, ${EMP}.${first.physicalColumn} ASC, ${EMP}.${number.physicalColumn} ASC, ${POS}.${validFrom.physicalColumn} DESC`,
    `FETCH FIRST ${P1_SURNAME_POSITION_MAX_ROWS} ROWS ONLY`,
  ].join('\n');
  ...
  if (sqlText.includes(`'${prefix}`) || sqlText.includes("'A%'") || /LIKE\s+'A%/i.test(sqlText)) {
    input.counters.employeePrefixEmbeddedInSql += 1;
    input.counters.unboundUserLiterals += 1;
  }
  if (/\bINNER\s+JOIN\b[\s\S]*NT_KP_KDR_STANOWISKA/i.test(sqlText)) {
    input.counters.innerJoinUsedForCurrentPosition += 1;
  }
  if (/\bINNER\s+JOIN\b[\s\S]*NT_KP_SLO_STANOWISKA/i.test(sqlText)) {
    input.counters.innerJoinUsedForPositionDictionary += 1;
  }
  // Temporal must not appear in WHERE body (ORDER BY may reference DATA_OD for sort)
  const whereIdx = sqlText.indexOf('\nWHERE ');
  const orderIdx = sqlText.indexOf('\nORDER BY ');
  const whereClause =
    whereIdx >= 0
      ? sqlText.slice(whereIdx, orderIdx >= 0 ? orderIdx : undefined)
      : '';
  if (/DATA_OD|DATA_DO|TRUNC\s*\(\s*SYSDATE\s*\)/i.test(whereClause)) {
    input.counters.employeeDroppedByTemporalWherePredicate += 1;
  }
  ...
  if (/\bMAX\s*\(\s*S02\.DATA_OD\s*\)/i.test(sqlText) || /\bMAX\s*\(\s*DATA_OD\s*\)/i.test(sqlText)) {
    input.counters.maxDateUsedToHideMultiplicity += 1;
    input.counters.firstCurrentPositionAutoSelected += 1;
  }
  if (/\bSELECT\s+DISTINCT\b/i.test(sqlText)) {
    input.counters.distinctUsedToHideMultiplicity += 1;
  }
  ...
  const sqlHash = sha256SurnamePosition(sqlText);
  ...
  const joinPlan = {
    drivingSource: `${P1_VERTICAL_OWNER}.${P1_VERTICAL_OBJECT}`,
    employeeToPosition: {
      joinType: 'left',
      on: [
      ...
      temporalPlacement: 'join_on',
    },
    positionToDictionary: {
      joinType: 'left',
      on: [`${POS}.${posId.physicalColumn} = ${DICT}.${dictId.physicalColumn}`],
    },
    whereOnly: [`UPPER(${EMP}.${last.physicalColumn}) LIKE :P001 ESCAPE '\\'`],
  };
  ...
  const compiled: TetaCompiledOracleSelect = {
    ...
    sourcePlanContractVersion: 'teta-aia-p1-surname-current-position-query-plan-v1',
    intent: P1_SURNAME_POSITION_INTENT,
    subject: P1_SURNAME_POSITION_SUBJECT,
    ...
    projections: [
      ...
      {
        ordinal: 5,
        businessRole: 'current_position_ref',
        resultAlias: 'CURRENT_POSITION_REF',
        expression: `${POS}.${posEmpRef.physicalColumn}`,
        sourceRole: 'current_position',
        logicalColumnNodeId: `oracle-column:${P1_VERTICAL_OWNER}:${P1_CURRENT_POSITION_SOURCE}:${posEmpRef.physicalColumn}`,
        accessColumnNodeId: `oracle-column:${P1_VERTICAL_OWNER}:${P1_CURRENT_POSITION_SOURCE}:${posEmpRef.physicalColumn}`,
        displayLabel: null,
      },
    ],
    joinTree: {
    ...
    predicates: [
      {
        ...
        kind: 'starts_with',
        sql: `UPPER(${EMP}.${last.physicalColumn}) LIKE :P001 ESCAPE '\\'`,
        placement: 'where',
        targetJoinId: null,
        accessColumnNodeIds: [
          `oracle-column:${P1_VERTICAL_OWNER}:${P1_VERTICAL_OBJECT}:${last.physicalColumn}`,
        ],
        bindNames: ['P001'],
      },
      {
  ...
  return { compiled, bindValues: { P001: bindValue }, sqlHash, temporalPredicates, joinPlan };
}
